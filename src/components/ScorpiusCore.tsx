...
function PetalBloom({
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      // Sections with photos/cards that should have alternating sphere positions:
      // 0: Hello Dark Forest (center)
      // 1: Hero intro (center)
      // 2: Quantum Threat Detection (right side for sphere)
      // 3: Blockchain Security (left side for sphere)
      // 4: Enterprise Command (right side for sphere)
      // 5: Features/Modules (left side for sphere)
      // 6: Enterprise Modules (right side for sphere)
      // 7: Testimonials (left side for sphere)
      // 8: Pricing (center)
      // 9: CTA (center)
      ...
      groupRef.current.position.z = Math.cos(scroll * Math.PI * 2) * 0.3;
    }
  });
